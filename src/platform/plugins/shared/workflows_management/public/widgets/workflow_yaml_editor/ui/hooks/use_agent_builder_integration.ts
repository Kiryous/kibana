    ...
    (options?: OpenAgentChatOptions) => {
      ...
      const pendingAutoSend =
        options?.initialMessage != null && options.initialMessage.length > 0
          ...
          : null;
      pendingAutoSendRef.current = pendingAutoSend;

      // A new conversation has no restored attachment to wait for, so attach the
      // current YAML immediately. `setChatConfig` keeps that attachment in sync
      // after the auto-send, including while these open flags are still in flight.
