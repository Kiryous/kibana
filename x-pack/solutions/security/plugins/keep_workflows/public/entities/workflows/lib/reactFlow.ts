...
export function handleSwitchNode(
  ...
  return {
    nodes: [switchStartNode, ...falseSubflowNodes, ...trueBranchNodes, switchEndNode],
    edges: [
      ...falseSubflowEdges,
      ...trueSubflowEdges,
...
export function handleDefaultNode(
  ...
  const nodes = [];
  let edges = [] as Edge[];
  const newNode = createDefaultNodeV2(step, nodeId, position, nextNodeId, prevNodeId, isNested);
...
export const processWorkflowV2 = (
  ...
  sequence?.forEach((step: any, index: number) => {
    ...
    position.y += 150;
    const { nodes, edges } = processStepV2(step, position, nextNodeId, prevNodeId, isNested);
    newNodes = [...newNodes, ...nodes];
    newEdges = [...newEdges, ...edges];
  });
