...
//   ) => Promise<CreateOrUpdateWorkflowResponse | null>;
//   deleteWorkflow: (workflowId: string, options?: DeleteOptions) => Promise<boolean>;
// }

// interface CreateOrUpdateWorkflowResponse {
//   workflow_id: string;
...
//         try {
//           const response = await api.request<CreateOrUpdateWorkflowResponse>(`/workflows`, {
//             method: 'POST',
//             body: formData,
//           });
...
//       const plural = uploadedWorkflowsIds.length === 1 ? 'workflow' : 'workflows';
//       revalidateLists();
//       showSuccessToast(`${uploadedWorkflowsIds.length} ${plural} uploaded successfully`);
...
//         const response = await api.request<CreateOrUpdateWorkflowResponse>('/workflows/json', {
//           method: 'POST',
//           body,
...
//     async (workflowId: string, definition: Definition | Record<string, unknown> | string) => {
//       try {
//         const body = getBodyFromStringOrDefinitionOrObject(definition);
...
//     async (workflowId: string, { skipConfirmation = false }: DeleteOptions = {}) => {
//       if (!skipConfirmation && !confirm('Are you sure you want to delete this workflow?')) {
//         return false;
//       }
//       try {
