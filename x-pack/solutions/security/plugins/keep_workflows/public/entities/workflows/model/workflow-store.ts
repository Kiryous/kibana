...
import { validateStepPure, validateGlobalPure, ValidationError } from '../lib/validate-definition';
...
import { V2StepTemplateSchema, V2StepTriggerSchema } from './schema';
import { parseWorkflow, wrapDefinitionV2 } from '../lib/parser';
...
function addNodeBetween(
  ...
  if (type === 'node') {
    ...
    if (!edge) {
      throw new KeepWorkflowStoreError(`Edge with target ${nodeOrEdgeId} not found`);
    }
  }
  ...
  if (type === 'edge') {
    ...
    if (!edge) {
      throw new KeepWorkflowStoreError(`Edge with id ${nodeOrEdgeId} not found`);
    }
  }
  ...
  if (!sourceId) {
    throw new KeepWorkflowStoreError(`Source is not defined for edge ${edge.id}`);
  }
  if (!targetId) {
    throw new KeepWorkflowStoreError(`Target is not defined for edge ${edge.id}`);
  }
  ...
  if (step.componentType === 'switch' && !canAddConditionBeforeEdge(sourceId, targetId)) {
  ...
  if (isTriggerComponent && nodes.find((node) => node && step.id === node.id)) {
    throw new KeepWorkflowStoreError(`Trigger of type ${step.type} is already in the workflow`);
  }

  const targetIndex = nodes.findIndex((node) => node.id === targetId);
  const sourceIndex = nodes.findIndex((node) => node.id === sourceId);
  if (targetIndex == -1) {
    throw new KeepWorkflowStoreError(`Target node with id ${targetId} not found`);
  }
...
export const useWorkflowStore = create<WorkflowState>()(
  devtools(
    (set, get) => ({
      ...
      triggerSave: () => set((state) => ({ saveRequestCount: state.saveRequestCount + 1 })),
      ...
      setLastDeployedAt: (deployedAt) => set({ lastDeployedAt: deployedAt, changes: 0 }),
      setSelectedEdge: (id) => {
      ...
      setInstalledProviders: (installedProviders: Provider[]) => set({ installedProviders }),
      ...
      updateDefinition: () => {
        ...
        const { sequence, properties: newProperties } = reConstructWorklowToDefinition({
          nodes,
          edges,
        ...
        const { isValid, validationErrors, canDeploy } = get().validateDefinition(definition);
      ...
      validateDefinition: (definition: Definition) => {
        ...
        const canDeploy =
          Object.values(validationErrors).filter(([_, severity]) => severity === 'error').length ===
          0;
      ...
      onConnect: (connection) => {
        ...
        const canConnect = (sourceNode: FlowNode | undefined, targetNode: FlowNode | undefined) => {
          ...
          if (sourceType === 'container' && sourceNode?.data?.type === 'foreach') {
            return true;
          }
      ...
      deleteNodes: (ids) => {
        ...
        finalEdges = edges.filter(
          (edge) => !(idArray.includes(edge.source) || idArray.includes(edge.target))
        );
        if (
          ['interval', 'alert', 'manual', 'incident'].includes(ids) &&
          edges.some((edge) => edge.source === 'trigger_start' && edge.target !== ids)
        ) {
        ...
        targets.forEach((edge) => {
          const target = edge.source === 'trigger_start' ? 'trigger_end' : edge.target;
      ...
      ) => initializeWorkflow(workflowId, { providers, installedProviders, secrets }, set, get),
    }),
    {
...
function onLayout(
  ...
  const { nodes: _layoutedNodes, edges: _layoutedEdges } = getLayoutedWorkflowElements(
    ns,
    es,
    opts
  );
...
function initializeWorkflow(
  ...
  const currentSelectedNode = get().selectedNode;
  const currentSelectedNodeStepName = get().nodes.find((node) => node.id === currentSelectedNode)
    ?.data?.name;
  ...
  set({ isLoading: true });
  const parsedWorkflow = definition?.value;
  ...
  const { nodes, edges } = processWorkflowV2(fullSequence, initialPosition, true);
  let newSelectedNodeId = null;
  if (isUpdatingExistingState && currentSelectedNode) {
    newSelectedNodeId =
      nodes.find((node) => node.data.name === currentSelectedNodeStepName)?.id ?? null;
  }
  set({
    workflowId,
