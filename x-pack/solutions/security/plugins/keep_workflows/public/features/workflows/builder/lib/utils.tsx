...
export function getToolboxConfiguration(providers: Provider[]): ToolboxConfiguration {
  /**
  ...
  for (const provider of providers) {
    if (provider.can_query) {
      steps.push({
        ...
        properties: {
          stepParams: provider.query_params?.filter((p) => p !== 'kwargs') ?? [],
        },
      });
    }
    if (provider.can_notify) {
      actions.push({
        ...
        properties: {
          actionParams: provider.notify_params?.filter((p) => p !== 'kwargs') ?? [],
        },
      });
    }
  }
...
export function edgeCanHaveAddButton(source: string, target: string) {
  let showAddButton =
    !source?.includes('empty') && !target?.includes('trigger_end') && source !== 'start';
...
export function canAddStepBeforeEdge(source: string, target: string) {
  return !source?.includes('empty') && !target?.includes('trigger_end') && source !== 'start';
}
