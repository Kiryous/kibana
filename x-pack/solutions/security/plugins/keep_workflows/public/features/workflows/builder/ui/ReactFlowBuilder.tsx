...
export const ReactFlowBuilder = () => {
  ...
  const reactFlowInstanceRef = useRef<ReactFlowInstance<FlowNode, Edge> | null>(null);
  ...
  useEffect(
    function fitViewOnLayoutAndEditorOpen() {
      ...
      const nodesToFit: Array<{ id: string }> = [];
      if (selectedNode) {
  ...
  return (
    <div
      css={css`
        ...
        display: flex;
      `}
      className="sqd-theme-light sqd-layout-desktop"
    >
      {isLayouted ? (
        <ReactFlow
          fitView
